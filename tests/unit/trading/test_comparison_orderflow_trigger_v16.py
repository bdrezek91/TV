...
from tradingview_mcp.core.backtest import comparison_orderflow_trigger_v16 as v16
...
def _bullish_inputs(*, extra_hours: int = 20) -> V16RawInputs:
...
def test_regime_gate_rejects_flat_no_edge_market() -> None:
    signal = detect_v16_signal("ETHUSDT", CUTOFF, _flat_inputs(), None)
    assert signal["eligible"] is False
    assert signal["primary_regime"] not in v16.ELIGIBLE_ENTRY_REGIMES
...
def test_entry_fires_confirmed_long_in_trend_up_regime() -> None:
    signal = detect_v16_signal("ETHUSDT", CUTOFF, _bullish_inputs(), None)
...
def test_entry_rejected_when_both_directions_confirmed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(v16.ofc, "confirm_setup", _always_confirmed)
    signal = detect_v16_signal("ETHUSDT", CUTOFF, _bullish_inputs(), None)
    assert signal["eligible"] is False
...
def test_atr_stop_matches_wilder_atr_and_is_independent_of_order_flow() -> None:
    ...
    signal = detect_v16_signal("ETHUSDT", CUTOFF, bullish, None)
    assert signal["eligible"] is True

    closed_1h = [c for c in bullish.candles_1h if c["open_time"] + dt.timedelta(hours=1) <= CUTOFF]
    closed_1h = closed_1h[-v16.CANDLE_WINDOW :]  # _build_feature_bundle windows to the same trailing count
    ...
    mutated_bundle = v16._build_feature_bundle(mutated, CUTOFF)
    ...
    assert mutated_bundle["volume"]["delta"] != v16._build_feature_bundle(bullish, CUTOFF)["volume"]["delta"]
...
def test_atr_stop_exit_fires_on_intrabar_breach() -> None:
    ...
    signal = detect_v16_signal("ETHUSDT", CUTOFF, inputs, None)
    ...
    result = simulate_v16_trade(signal, crashed_inputs, None)
...
def test_orderflow_reversal_exit_fires_when_net_score_drops() -> None:
    ...
    result = simulate_v16_trade(signal, inputs, None)
    assert result.exit_reason == "ORDERFLOW_REVERSAL"
    assert result.exit_time is not None
    assert result.exit_time < CUTOFF + dt.timedelta(hours=v16.MAX_HOLD_HOURS)
...
def test_costs_and_funding_match_frozen_formula() -> None:
    ...
    risk_amount = v16.CAPITAL * v16.RISK_PCT / Decimal("100")
    ...
    expected_fees = (entry_notional + exit_notional) * v16.TAKER_FEE_RATE_PCT / Decimal("100")
    expected_slippage = (entry_notional + exit_notional) * v16.SLIPPAGE_BPS_EACH_SIDE / Decimal("10000")
    ...
    expected_funding = (
        entry_notional * v16.CONSERVATIVE_FUNDING_BPS_PER_8H / Decimal("10000") * expected_funding_intervals
    )
...
def test_simulate_marks_unfilled_when_no_future_candles() -> None:
    ...
    inputs = V16RawInputs(candles_4h=[], candles_1h=[], candles_15m=[], trades_1m=[], derivatives=[])
    result = simulate_v16_trade(signal, inputs, None)
...
def test_simulate_rejects_ineligible_signal() -> None:
    ...
    with pytest.raises(ValueError, match="ineligible"):
        simulate_v16_trade({"eligible": False}, inputs, None)


def test_v16_development_rejects_reserved_mr_v2_holdout(tmp_path: Path) -> None:
    reserved = tmp_path / "holdout_mrv2_120d" / "cache"
    with pytest.raises(ValueError, match="reserved holdout"):
        validate_v16_development_cache_root(reserved)
