"""Development-only runner for frozen V17 order-flow-triggered, regime-gated
single-instrument strategy.

The runner reads already-observed extended-history caches, refuses the
reserved MR V2 holdout, and cannot promote V17. Each of the five frozen
...
DEFAULT_OUTPUT_NAME = "research_v17_orderflow_trigger_development.json"
...
def _inputs_from_bundle(bundle: Any) -> V17RawInputs:
    return V17RawInputs(
...
def _collect_symbol(
    ...
    for cutoff in cutoffs:
        if busy_until is not None and cutoff < busy_until:
            ...
            continue
        signal = detect_v17_signal(symbol, cutoff, inputs, None if is_btc else btc_inputs)
        if not signal.get("eligible"):
            funnel[f"reject:{signal.get('reason', 'unknown')}"] += 1
            continue
        funnel["eligible_signal"] += 1
        result = simulate_v17_trade(signal, inputs, None if is_btc else btc_inputs)
        events.append(
            {
                "symbol": symbol,
...
def assess_v17_development(
    ...
    return {
        ...
        "eligible_to_freeze_for_new_v17_holdout": passed,
        "checks": checks,
        "schedule_metrics": metrics,
        "interpretation": "PASS permits freezing for a new V17-specific holdout; it never promotes live trading.",
    }


def main(args: argparse.Namespace) -> dict[str, Any]:
    cache_root = validate_v17_development_cache_root(Path(args.cache_root))
    summary = json.loads(
    ...
    if missing:
        raise ValueError(
            f"V17 development requires frozen five-symbol universe; missing {missing}"
        )
    ...
    payload = {
        ...
        "development_assessment": assess_v17_development(results, cadence),
        "guards": [
            "reserved holdout_mrv2_120d path is rejected",
            "closed 1h candles / trade aggregates / derivatives only, no lookahead",
            "one frozen V17 hypothesis, single-leg, five symbols evaluated independently",
            ...
            "development output cannot promote V17",
        ],
    }
    output = (
