#!/usr/bin/env python3
"""Candidate V6: Momentum Continuation comparison.
...
from tradingview_mcp.core.backtest.comparison_momentum_v1 import evaluate_momentum_continuation
...
VARIANTS = {
    ...
    "MOMENTUM_ONLY": {"momentum_continuation"},
    "MEAN_REVERSION_PLUS_MOMENTUM": {"mean_reversion", "momentum_continuation"},
}
...
def _run_schedule(schedule_name: str, hours: tuple[int, ...], requested: list[str], successful: dict, start: dt.datetime, end: dt.datetime) -> dict[str, Any]:
    ...
    for cutoff in cutoffs:
        ...
        for symbol in requested:
            ...
            momentum = evaluate_momentum_continuation(chain, btc_regime=btc_regime)
            ...
            if direction in {"LONG", "SHORT"}:
                ...
                confirmation = momentum.get("confirmation") or {}
                risk = momentum.get("risk") or {}
            ...
            if signal is not None:
                ...
                result = simulate_neutral_signal(signal, future)
                events.append({"symbol": symbol, "decision_time": signal.decision_time, "setup": "momentum_continuation", "observation": result.observation, "order": result.order})
    ...
    return {
        ...
        "momentum_funnel": dict(funnel),
        "momentum_active_by_w1_regime": dict(by_regime),
        "momentum_active_by_direction": dict(by_direction),
        "momentum_w3": dict(w3),
        "momentum_w4": dict(w4),
        "variants": {name: _lifecycle_variant(events, allowed, end) for name, allowed in VARIANTS.items()},
        "interpretation_contract": "Momentum generation uses only decision-time closed candles. W3/W4/W5 are unchanged. Combined same-symbol/same-scan cross-setup conflicts are excluded instead of prioritized.",
    }
...
def main(args) -> dict[str, Any]:
    ...
    payload = {
        ...
        "candidate_kind": "RESEARCH_V2_CANDIDATE_V6_MOMENTUM_CONTINUATION",
        ...
        "momentum_definition": {
            ...
            "impulse_atr_range": ["0.90", "2.75"],
            "min_directional_candles": 3,
            "close_location_min": "0.70",
            "entry_pullback_atr": "0.20",
            "max_stop_distance_atr": "1.60",
        ...
        "guards": [
            ...
            "momentum has a fixed predeclared price-only detection rule",
    ...
    write_json_cache(out_dir / "research_v2_candidate_v6_momentum.json", payload)
    return payload
