...
from tradingview_mcp.core.backtest.comparison_vwap_reversion_v1 import evaluate_vwap_deviation_reversion
...
VARIANTS = {
    ...
    "VWAP_REVERSION_ONLY": {"vwap_deviation_reversion"},
    "MEAN_REVERSION_PLUS_VWAP_REVERSION": {"mean_reversion", "vwap_deviation_reversion"},
}
...
def _run_schedule(schedule_name: str, hours: tuple[int, ...], requested: list[str], successful: dict, start: dt.datetime, end: dt.datetime) -> dict[str, Any]:
    ...
    funnel, w3, w4, by_regime, by_direction = Counter(), Counter(), Counter(), Counter(), Counter()
    ...
    for cutoff in cutoffs:
        ...
        for symbol in requested:
            ...
            candidate = evaluate_vwap_deviation_reversion(chain, btc_regime=btc_regime)
            ...
            if signal is not None:
                ...
                result = simulate_neutral_signal(signal, future)
                events.append({"symbol": symbol, "decision_time": signal.decision_time, "setup": "vwap_deviation_reversion", "observation": result.observation, "order": result.order})
    ...
    return {
        ...
        "vwap_funnel": dict(funnel),
        "vwap_active_by_w1_regime": dict(by_regime),
        "vwap_active_by_direction": dict(by_direction),
        "vwap_w3": dict(w3),
        "vwap_w4": dict(w4),
...
def main(args) -> dict[str, Any]:
    ...
    payload = {
        ...
        "candidate_kind": "RESEARCH_V2_CANDIDATE_V8_VWAP_DEVIATION_REVERSION",
        "status": "RESEARCH_ONLY_NOT_PROMOTED",
        "read_only": True,
        "evaluation_window": {"start": start, "end": end},
        "requested_symbols": requested,
        "vwap_definition": {
            "rolling_15m_candles": 96,
            "min_deviation_atr": "1.25",
            "max_deviation_atr": "4.00",
            "requires_rejection_candle": True,
        ...
        "guards": [
            ...
            "VWAP setup uses only closed decision-time candles",
    ...
    write_json_cache(out_dir / "research_v2_candidate_v8_vwap_reversion.json", payload)
    return payload
