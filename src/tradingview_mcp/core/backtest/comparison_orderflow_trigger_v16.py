"""Frozen development contract for V16 order-flow-triggered, regime-gated
single-instrument strategy.

Unlike V15 (two synchronized legs vs BTC), V16 trades each of the five
...
volume/futures/liquidations/orderbook; the stop is `1.5 * ATR(14)` on 1h
...
Known development-cache limitation (see V16_PREREGISTRATION.md): historical
...
V16_NAME = "V16_ORDERFLOW_TRIGGER_SINGLE_INSTRUMENT"
V16_SETUP = "orderflow_trigger_regime_gated"
BTC_SYMBOL = "BTCUSDT"
...
def validate_v16_development_cache_root(cache_root: Path) -> Path:
    ...
    if marker:
        raise ValueError(
            f"V16 development runner refuses reserved holdout cache: {marker}"
        )
    return resolved
...
class V16RawInputs:
    ...
    keeps the fields V16 actually needs -- no 1m candles, no orderbook/
...
def _build_feature_bundle(inputs: V16RawInputs, cutoff: dt.datetime) -> dict[str, Any]:
...
def _btc_regime_at(
    symbol: str, cutoff: dt.datetime, btc_inputs: Optional[V16RawInputs]
) -> Optional[str]:
...
def detect_v16_signal(
    symbol: str,
    cutoff: dt.datetime,
    inputs: V16RawInputs,
    btc_inputs: Optional[V16RawInputs],
) -> dict[str, Any]:
    """Return the frozen V16 decision at `cutoff`, using only closed rows.
    ...
    since V16 has no Warstwa 2 setup of its own). CONFIRMED in exactly one
    direction fires a signal; CONFIRMED in both is rejected as ambiguous.
    """
    symbol = symbol.upper()
    if symbol not in REQUIRED_SYMBOLS:
        raise ValueError(f"unsupported V16 symbol: {symbol}")
...
def simulate_v16_trade(
    ...
    if not signal.get("eligible"):
        raise ValueError("cannot simulate an ineligible V16 signal")
    ...
    if exit_time is None or exit_price is None or exit_reason is None:
        observation = TradeObservation(
            ...
            setup_name=V16_SETUP,
            direction=direction,
            filled=False,
            r_multiple=None,
        )
        return V16ExecutionResult(
